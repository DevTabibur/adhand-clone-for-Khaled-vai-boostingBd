import type { Translations } from "../types"

export const translations: Translations = {
  en: {
    nav: {
      platforms: "Platforms",
      freelancers: "Freelancers",
      testimonials: "Testimonials",
      blog: "Blog",
      signIn: "Sign in",
      signUp: "Sign up",
    },
    hero: {
      title: "Promote your business in Meta, TikTok and Telegram",
      subtitle: "Find clients on booming platforms with AdHand — easy payment and ad launch in one interface",
      cta: "Sign up",
    },
    trusted: "We are trusted by leading companies",
  },
  bn: {
    nav: {
      platforms: "প্ল্যাটফর্ম",
      freelancers: "ফ্রিল্যান্সার",
      testimonials: "প্রশংসাপত্র",
      blog: "ব্লগ",
      signIn: "সাইন ইন",
      signUp: "সাইন আপ",
    },
    hero: {
      title: "মেটা, টিকটক এবং টেলিগ্রামে আপনার ব্যবসার প্রচার করুন",
      subtitle: "AdHand দিয়ে বুমিং প্ল্যাটফর্মে ক্লায়েন্ট খুঁজুন — সহজ পেমেন্ট এবং একটি ইন্টারফেসে বিজ্ঞাপন চালু",
      cta: "সাইন আপ",
    },
    trusted: "আমরা শীর্ষস্থানীয় কোম্পানিগুলির দ্বারা বিশ্বস্ত",
  },
  ru: {
    nav: {
      platforms: "Платформы",
      freelancers: "Фрилансеры",
      testimonials: "Отзывы",
      blog: "Блог",
      signIn: "Войти",
      signUp: "Регистрация",
    },
    hero: {
      title: "Продвигайте свой бизнес в Meta, TikTok и Telegram",
      subtitle:
        "Находите клиентов на растущих платформах с AdHand — простая оплата и запуск рекламы в одном интерфейсе",
      cta: "Регистрация",
    },
    trusted: "Нам доверяют ведущие компании",
  },
  ar: {
    nav: {
      platforms: "المنصات",
      freelancers: "المستقلون",
      testimonials: "الشهادات",
      blog: "المدونة",
      signIn: "تسجيل الدخول",
      signUp: "التسجيل",
    },
    hero: {
      title: "روج لعملك في Meta و TikTok و Telegram",
      subtitle: "اعثر على العملاء في المنصات المزدهرة مع AdHand — دفع سهل وإطلاق الإعلانات في واجهة واحدة",
      cta: "التسجيل",
    },
    trusted: "نحن موثوقون من قبل الشركات الرائدة",
  },
  hi: {
    nav: {
      platforms: "प्लेटफॉर्म",
      freelancers: "फ्रीलांसर",
      testimonials: "प्रशंसापत्र",
      blog: "ब्लॉग",
      signIn: "साइन इन",
      signUp: "साइन अप",
    },
    hero: {
      title: "Meta, TikTok और Telegram में अपने व्यवसाय का प्रचार करें",
      subtitle: "AdHand के साथ बूमिंग प्लेटफॉर्म पर क्लाइंट खोजें — आसान भुगतान और एक इंटरफेस में विज्ञापन लॉन्च",
      cta: "साइन अप",
    },
    trusted: "हम अग्रणी कंपनियों द्वारा भरोसेमंद हैं",
  },
  fr: {
    nav: {
      platforms: "Plateformes",
      freelancers: "Freelancers",
      testimonials: "Témoignages",
      blog: "Blog",
      signIn: "Se connecter",
      signUp: "S'inscrire",
    },
    hero: {
      title: "Promouvez votre entreprise sur Meta, TikTok et Telegram",
      subtitle:
        "Trouvez des clients sur les plateformes en plein essor avec AdHand — paiement facile et lancement de publicités dans une seule interface",
      cta: "S'inscrire",
    },
    trusted: "Nous sommes approuvés par les entreprises leaders",
  },
}
